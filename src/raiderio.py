...
async def guild_profile():
    try:
        url = (
            BASE_REQUEST_URL
            + "guilds/profile?region=eu&realm=eredar&name=casual%20progress&fields=raid_progression"
        )
        raid_progression = getRIOData(url)
        difficulty = None
        boss_id = (
            int(raid_progression["raid_progression"].values()[0]["summary"][0]) - 1
        )
        ...
        url = (
            BASE_REQUEST_URL
            + "guilds/boss-kill?region=eu&realm=eredar&\
            guild=casual%20progress&raid="
            + static_raiding_data["raids"][0]["slug"]
            + "&\
            boss="
            + static_raiding_data["raids"][0]["encounters"][boss_id]["slug"]
            + "&difficulty="
            + difficulty
        )
